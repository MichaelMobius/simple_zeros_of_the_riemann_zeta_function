...
pressure-preserving refinement.  It includes both the exact unit-diagonal
2-by-2 identity and a robust finite-Gram lower bound requiring only trace at
most two.
-/
...
/-- A PSD 2-by-2 matrix whose real trace is at most two also has all
its eigenvalues in `[0,2]`.  This is the finite-Gram version needed by v17. -/
theorem v17_fin2_eigenvalue_le_two_of_rtrace_le
    (M : Matrix (Fin 2) (Fin 2) ℂ)
    (hM : M.PosSemidef)
    (htrace : RHLinalg.rtrace M ≤ 2) :
    ∀ i : Fin 2, hM.isHermitian.eigenvalues i ≤ 2 := by
  have hsum : ∑ i : Fin 2, hM.isHermitian.eigenvalues i ≤ 2 := by
    rw [← RHLinalg.rtrace_eq_sum_eigenvalues hM.isHermitian]
    exact htrace
  have h0 := hM.eigenvalues_nonneg (0 : Fin 2)
  have h1 := hM.eigenvalues_nonneg (1 : Fin 2)
  intro i
...
/-- Robust finite-Gram 2-by-2 lower bound.  Exact unit diagonal is unnecessary:
PSD plus `rtrace ≤ 2` prevents eigenvalues above the breakpoint, so the full
centered Frobenius energy is the defect and in particular dominates the two
off-diagonal entries. -/
theorem v17_fin2_gram_defect_ge_two_offdiag_of_rtrace_le
    (M : Matrix (Fin 2) (Fin 2) ℂ)
    (hM : M.PosSemidef)
    (htrace : RHLinalg.rtrace M ≤ 2) :
    2 * ‖M 0 1‖ ^ 2 ≤ gramSpectralDefect M hM := by
  have hle := v17_fin2_eigenvalue_le_two_of_rtrace_le M hM htrace
  have hle0 : ∀ k : Fin 2, hM.isHermitian.eigenvalues₀ k ≤ 2 := by
    intro k
    rw [← RHLinalg.eigenvalues_eigEquiv hM.isHermitian k]
    exact hle _
  ...
  rw [v17_frobSq_eq_sum_norm_sq]
  simp only [Fin.sum_univ_two]
  simp [Matrix.sub_apply, Matrix.smul_apply, Matrix.one_apply, h10]
  nlinarith [sq_nonneg ‖M 0 0 - 1‖, sq_nonneg ‖M 1 1 - 1‖]
