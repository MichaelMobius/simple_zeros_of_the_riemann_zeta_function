#!/usr/bin/env python3
"""Generate full Package-E3 R2 survivor contraction certificates.

R2 starts from the 23 exact R1-contracted boxes.  Exactly five words survive
that round.  For each survivor this generator kernelizes the same historical
rational-bootstrap step:

* the R2 symbolic-root 21-block quadratic is below the true gap functional on
  the already-certified R1 contracted box;
...
* weighted Cauchy contracts all 21 consecutive block sums to the exact rational
  intervals used by the final-five Package E bridge.
...
def exact_r2_survivors():
    ...
    r1_map = {tuple(word): intervals for word, intervals, _qmin, _nused in r1}
    out = []
    for word, contracted, qmin, nused in r2:
        out.append((word, r1_map[tuple(word)], contracted, qmin, nused))
    return out
...
def emit_survivor(word, intervals, contracted, qmin_expected, nused_expected):
    ...
    input_box = f"v26E3R1_{wc}ContractedBox"
    ...
    for r in range(1, 7):
        for i in range(7 - r):
            ...
            if (i, r) in used_by_key:
                ...
                proof = (
                    f"  exact v26_R2_cell_{cell_idx:04d} (x := {s}) hblock.1 hblock.2"
                )
                used_count += 1
            else:
                expr = "0"
                proof = f"  exact v26_limitingWeight_nonneg ({s})"
            thm = f"v26_E3_R2_{wc}_minor_{idx21:02d}"
...
''')
    ...
    for r in range(1, 7):
        for i in range(7 - r):
            ...
            thm = f"v26_E3_R2_{wc}_contract_b{r-1}{i}"
...
''')
    ...
    imports = {
        f"import HurtadoZeta23.V26R1Survivor{wc}Generated",
...
'''
    src += f'''/-- Historical rounded R2 quadratic for survivor `{wc}`. -/
...
/-- Exact LDL completion for R2 survivor `{wc}`. -/
theorem v26_E3_R2_{wc}_completion
    (x0 x1 x2 x3 x4 x5 : ℝ) :
...
/-- Symbolic-root 21-block analytic assembly for R2 survivor `{wc}`. -/
def {assembly_def} (x0 x1 x2 x3 x4 x5 : ℝ) : ℝ :=
  {assembly}

/-- Pure algebra: the rounded R2 quadratic is the symbolic block assembly. -/
theorem v26_E3_R2_{wc}_Q_eq_assembled
    (x0 x1 x2 x3 x4 x5 : ℝ) :
    {q_def} x0 x1 x2 x3 x4 x5 =
...
theorem v26_E3_R2_{wc}_assembled_le_gapF
    (x0 x1 x2 x3 x4 x5 : ℝ)
...
theorem v26_E3_R2_{wc}_Q_le_gapF
    (x0 x1 x2 x3 x4 x5 : ℝ)
    (hbox : {input_box} x0 x1 x2 x3 x4 x5) :
    {q_def} x0 x1 x2 x3 x4 x5 ≤
      v26GapF limitingWeight x0 x1 x2 x3 x4 x5 := by
  rw [v26_E3_R2_{wc}_Q_eq_assembled]
  exact v26_E3_R2_{wc}_assembled_le_gapF x0 x1 x2 x3 x4 x5 hbox

/-- Strict counterexamples in survivor `{wc}` have energy below delta-qmin. -/
theorem v26_E3_R2_{wc}_energy_lt
    (x0 x1 x2 x3 x4 x5 : ℝ)
    (hbox : {input_box} x0 x1 x2 x3 x4 x5)
    (hbad : v26GapF limitingWeight x0 x1 x2 x3 x4 x5 < v26Delta) :
    {energy_def} x0 x1 x2 x3 x4 x5 < {ql(gap)} := by
  have hdom := v26_E3_R2_{wc}_Q_le_gapF x0 x1 x2 x3 x4 x5 hbox
  rw [v26_E3_R2_{wc}_completion] at hdom
  norm_num [v26Delta, {qmin_def}] at hdom hbad ⊢
  linarith

theorem v26_E3_R2_{wc}_energy_le
    (x0 x1 x2 x3 x4 x5 : ℝ)
    (hbox : {input_box} x0 x1 x2 x3 x4 x5)
    (hbad : v26GapF limitingWeight x0 x1 x2 x3 x4 x5 < v26Delta) :
    {energy_def} x0 x1 x2 x3 x4 x5 ≤ {ql(gap)} :=
  le_of_lt (v26_E3_R2_{wc}_energy_lt x0 x1 x2 x3 x4 x5 hbox hbad)
...
/-- Full exact 21-block R2 contraction for final survivor `{wc}`. -/
theorem v26_E3_R2_{wc}_contract_all
    (x0 x1 x2 x3 x4 x5 : ℝ)
    (hbox : {input_box} x0 x1 x2 x3 x4 x5)
    (hbad : v26GapF limitingWeight x0 x1 x2 x3 x4 x5 < v26Delta) :
    {output_box} x0 x1 x2 x3 x4 x5 := by
  have henergy := v26_E3_R2_{wc}_energy_le x0 x1 x2 x3 x4 x5 hbox hbad
  unfold {input_box} at hbox
  rcases hbox with ⟨{hb_names}⟩
{contract_calls}
  unfold {output_box}
...
def write(out_dir: Path):
    ...
    survivors = exact_r2_survivors()
    modules = []
    codes = []
    files = {}
    for word, intervals, contracted, qmin, nused in survivors:
        wc = wcode(word)
        mod = f"V26R2Survivor{wc}Generated"
    ...
    assert tuple(codes) == boot.EXPECTED_FINAL
    agg = out_dir / "V26R2SurvivorContractionsGenerated.lean"
    agg.write_text(
        "\n".join(f"import HurtadoZeta23.{m}" for m in modules)
        + "\n\nnamespace HurtadoZeta23\n"
        + "/-- Marker: all five R2 survivor contraction modules compiled. -/\n"
        + "theorem v26_E3_R2_all_survivor_contractions_loaded : True := by trivial\n"
        + "end HurtadoZeta23\n",
        encoding="utf-8",
    )
    raw = agg.read_bytes()
    ...
    manifest = {
        "survivor_count": len(modules),
        "codes": codes,
        "modules": modules,
        "files": files,
    }
    (out_dir / "v26_r2_survivor_contractions_manifest.json").write_text(
...
def main():
    ...
    print("R2 SURVIVOR CONTRACTION GENERATION OK")
    print("survivors:", m["survivor_count"])
    print("codes:", ", ".join(m["codes"]))
