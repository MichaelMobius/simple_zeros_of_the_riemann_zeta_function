#!/usr/bin/env python3
"""Generate kernel-checkable analytic bridges for all 280 R0 discards.

For each discarded Package-D basin word this generator emits:
  * the concrete word;
  * the exact symbolic-root 21-block lower assembly;
  * Q = assembly by isolated ring normalization;
  * assembly <= actual v26GapF from the reusable R0 analytic-cell catalog;
  * exclusion of a strict counterexample using the exact bootstrap discard
    theorem generated by `generate_v26_bootstrap_lean.py`.

Python remains untrusted: all analytic and algebraic claims are rechecked by
Lean.  The generated modules use a larger heartbeat budget because a small
number of 21-block assemblies (notably code 111611) exceed Lean's default
200k elaboration budget even though all constituent cell theorems are already
kernel-checked.
"""
