#!/usr/bin/env python3
"""Generate kernel-checkable analytic bridges for all 18 R2 discards.

R2 starts from the exact 21-block contracted boxes proved for the 23 R1
survivors.  For each of the 18 historical third-pass discards this generator
emits:
  * the exact symbolic-root R2 21-block assembly;
  * equality with the already generated historical R2 bootstrap quadratic;
  * analytic domination by the true six-gap functional using the deduplicated
    R2 cell catalog;
  * exclusion of a strict counterexample via the exact rational R2 discard
    certificate.
...
import generate_v26_r2_cell_catalog as catalog
...
def exact_r2_data():
    ...
    r1_map = {tuple(item[0]): item[1] for item in r1}
    out = []
    for idx, (word, _margin, qmin, nused) in enumerate(d2):
        intervals = r1_map[tuple(word)]
...
def emit_word(discard_index: int, word, intervals, used) -> tuple[str, set[int]]:
    ...
    for r in range(1, 7):
        for i in range(7 - r):
            ...
            if (i, r) in used_by_key:
                ...
                hn = f"hm{idx21:02d}"
                proofs.append(
                    f"  have {hn} := v26_R2_cell_{cell_idx:04d} (x := {s}) {hb}.1 {hb}.2"
                )
                used_count += 1
            else:
                expr = "0"
    ...
    assembled = f"v26E3R2Assembled_{wc}"
    lower = assembled_lower(exprs)
    hb_names = ", ".join(f"hb{i:02d}" for i in range(21))

    src = f'''/-- Historical R2 block-minorant assembly for discarded word `{wc}`. -/
def {assembled} (x0 x1 x2 x3 x4 x5 : ℝ) : ℝ :=
  {lower}

/-- Pure algebra: the exact historical R2 quadratic for `{wc}` is the symbolic
21-block assembly. -/
theorem v26_E3_R2_Q_eq_assembled_{wc}
    (x0 x1 x2 x3 x4 x5 : ℝ) :
    v26Q_R2_{wc} x0 x1 x2 x3 x4 x5 =
      {assembled} x0 x1 x2 x3 x4 x5 := by
  unfold v26Q_R2_{wc} {assembled}
  simp [v26Pressure, v21RootRight]
  ring

/-- The R2 assembly for `{wc}` is below the true functional throughout its
exact R1-contracted 21-block box. -/
theorem v26_E3_R2_assembled_le_gapF_{wc}
    (x0 x1 x2 x3 x4 x5 : ℝ)
    (hbox : v26E3R1_{wc}ContractedBox x0 x1 x2 x3 x4 x5) :
    {assembled} x0 x1 x2 x3 x4 x5 ≤
      v26GapF limitingWeight x0 x1 x2 x3 x4 x5 := by
  unfold v26E3R1_{wc}ContractedBox at hbox
...
/-- Analytic domination of the exact R2 bootstrap quadratic for `{wc}`. -/
theorem v26_E3_R2_Q_le_gapF_{wc}
    (x0 x1 x2 x3 x4 x5 : ℝ)
    (hbox : v26E3R1_{wc}ContractedBox x0 x1 x2 x3 x4 x5) :
    v26Q_R2_{wc} x0 x1 x2 x3 x4 x5 ≤
      v26GapF limitingWeight x0 x1 x2 x3 x4 x5 := by
  rw [v26_E3_R2_Q_eq_assembled_{wc}]
  exact v26_E3_R2_assembled_le_gapF_{wc} x0 x1 x2 x3 x4 x5 hbox

/-- R2 discarded word `{wc}` contains no strict counterexample once its exact
R1-contracted input box has been established. -/
theorem v26_E3_R2_no_counterexample_{wc}
    (x0 x1 x2 x3 x4 x5 : ℝ)
    (hbox : v26E3R1_{wc}ContractedBox x0 x1 x2 x3 x4 x5) :
    ¬ v26GapF limitingWeight x0 x1 x2 x3 x4 x5 < v26Delta := by
  intro hbad
  have hdom := v26_E3_R2_Q_le_gapF_{wc} x0 x1 x2 x3 x4 x5 hbox
  have hdiscard := v26_discard_R2_{wc} x0 x1 x2 x3 x4 x5
  linarith
'''
...
def write(out_dir: Path):
    ...
    for start in range(0, len(items), CHUNK_SIZE):
        ...
        name = f"V26R2AnalyticDiscards{idx:02d}Generated"
        bodies = []
        cats: set[int] = set()
        imports = {
        ...
        for discard_index, word, intervals, used in chunk_items:
            wc = wcode(word)
            imports.add(f"import HurtadoZeta23.V26BootstrapR2Chunk{discard_index // boot.CHUNK_SIZE}")
            imports.add(f"import HurtadoZeta23.V26R1Survivor{wc}Generated")
            body, cmods = emit_word(discard_index, word, intervals, used)
            bodies.append(body)
            cats |= cmods
        for m in cats:
            imports.add(f"import HurtadoZeta23.V26R2CellCatalog{m:03d}Generated")
        header = "\n".join(sorted(imports)) + '''
...
'''
    ...
    assert len(items) == 18 and len(modules) == 2
    agg = "\n".join(f"import HurtadoZeta23.{m}" for m in modules)
    agg += '''

namespace HurtadoZeta23
/-- Marker: all 18 R2 analytic discard bridges compiled. -/
theorem v26_E3_R2_all_discards_loaded : True := by trivial
end HurtadoZeta23
'''
    agg_path = out_dir / "V26R2AnalyticDiscardsGenerated.lean"
    ...
    (out_dir / "v26_r2_analytic_discards_manifest.json").write_text(
...
def main():
    ...
    print("R2 ANALYTIC DISCARD GENERATION OK")
